...
let citations = [
  ...
  let message = document.querySelector("#citation")
  let auteur = document.querySelector("#auteur")

  button.addEventListener("click", newCitation)

  function newCitation() {
    newMessage = Math.round((Math.random()*citations.length));
    citation();
    author();
    console.log(newMessage)
  }
  function citation() {
    message.textContent = `${citations[newMessage][0]}`
  }
  function author() {
    auteur.textContent = `${citations[newMessage][1]}`
  }
/*   console.log(citations[20][1])
